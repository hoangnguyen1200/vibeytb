...
export const VideoScriptSchema = z.object({
  ...
  scenes: z.array(z.object({
    ...
    target_website_url: z.string().nullable().optional().describe("CRITICAL: To avoid Cloudflare anti-bot blocks, DO NOT provide web-app URLs that require login or heavy verification (like https://www.google.com/url?sa=E&source=gmail&q=chatgpt.com or perplexity.ai/search). INSTEAD, provide the Marketing Landing Page, About page, or Blog page of the tool (e.g., openai.com, perplexity.ai/about, github.com). If the tool is strictly locked, choose a different tool."),
    target_search_query: z.string().nullable().optional().describe("A short, highly relevant search query (3-6 words) related to the scene's narration. This will be auto-typed into the website's search bar. Example: if narration is about AI music, query should be 'AI music generator'. If target_website_url is null, this MUST be null."),
...
- \`target_website_url\`: Out of the 4-5 total scenes, EXACTLY 1 OR 2 scenes MUST have a valid homepage URL in this field (e.g., "https://www.chatpdf.com" or "https://gamma.app"). This is absolutely vital because our headless browser will visit this URL to record the screen UI.
